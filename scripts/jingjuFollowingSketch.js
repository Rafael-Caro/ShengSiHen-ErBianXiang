...
function setup () {
  ...
  voiceSlider = createSlider(0, 100)
    ...
    .parent("sketch-holder");
  accToggle = createCheckbox(' accomp.', true)
  ...
  headingLeft = leftExtraSpace + 120;
...
function draw () {
  ...
  textSize(18);
  if (lang == "zh") {
    var subtitlePos = headingX - textWidth(play[lang] + ' (' + character.zh + ')') / 2
    stroke(0)
    strokeWeight(1);
    textStyle(NORMAL);
    text(play[lang], subtitlePos, topExtraSpace+50);
    noStroke();
    text(' (' + character.zh + ')', subtitlePos+textWidth(play[lang]), topExtraSpace+50);
    // noStroke();
    // text(play[lang], headingX-textWidth(' (' + character.zh + ')')/2, topExtraSpace+50);
    // text(' (' + character.zh + ')', headingX+textWidth(' (' + character.zh + ')')/2, topExtraSpace+50);
  } else {
    var subtitlePos = headingX - textWidth(play[lang] + ' (' + character.wy + ')') / 2
    stroke(0)
    strokeWeight(1);
    textStyle(ITALIC);
    text(play[lang], subtitlePos, topExtraSpace+50);
    noStroke();
    textStyle(NORMAL);
    text(' (' + character.wy + ')', subtitlePos+textWidth(play[lang]), topExtraSpace+50);
  }
   // + ' (' + character.wy + ')';
  noStroke();
  credits.display();
...
function start () {
  ...
  lang = version;
  var mbid = "shengsihen-erbianxiang"
  audioLoader(mbid);
  loaded = false;
  playing = false;
...
function CreateLyricsBox (lyrics, i) {
  ...
  this.update = function () {
    if (cursor.x >= this.x1 && cursor.x < this.x2) {
      ...
      if (this.type == 'O') {
        this.fill = color (255, 140, 0, 150);
        this.stroke = color (255, 140, 0, 150);
        this.txtBack = color (255, 140, 0, 70);
      } else {
        this.fill = color(0, 100, 0, 120);
        this.stroke = color(0, 100, 0, 120);
        this.txtBack = color(0, 100, 0, 50);
      }
      ...
      // this.fill = color(0, 50);
      if (this.type == 'O') {
        this.fill = color (255, 140, 0, 50);
        // this.stroke = color (255, 140, 0, 50);
      } else {
        this.fill = color(0, 100, 0, 50);
        // this.stroke = color(0, 100, 0, 50);
      }
      noStroke();
...
function CreateClock () {
  ...
  console.log(trackDuration, this.total);
  this.now;
  this.display = function() {
    ...
    noStroke();
    fill(50);
    text(this.clock, headingLeft, lyricsBoxBottom-30);
  }
}
